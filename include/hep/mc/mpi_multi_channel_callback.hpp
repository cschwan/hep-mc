...
/// MPI equivalent of \ref multi_channel_default_callback.
///
...
/// function always returns `true`. It is the equivalent of \ref multi_channel_verbose_callback and
/// only writes an output if it was called from rank zero to avoid duplicated output.
///
...
/// mpi_multi_channel_default_callback which does nothing. The callback function can e.g. be set to
/// \ref mpi_multi_channel_verbose_callback which prints detailed results after each iteration.
///
...
inline mpi_multi_channel_callback_type<T> mpi_multi_channel_callback(
    ...
    static mpi_multi_channel_callback_type<T> object = mpi_multi_channel_default_callback<T>;
