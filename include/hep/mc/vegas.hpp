 ...
 *
 * The parameter `total_calls` determines the sample size \f$ N \f$ of the iteration
 * \ref vegas_iteration performs and therefore usually has usually the same value as `calls`. If
 * VEGAS is run in parallel, then this function will be called multiple times with a differently
 * seeded generator and with `calls` parameters each smaller than `total_calls` but their sum being
 * equal to `total_calls`.
 */
 ...
 * iteration specified by the corresponding value in `iteration_calls`. The pdf that is sampled from
 * has `bins` for each dimension as is refined \ref vegas_refine_pdf using the
 * \f$ \alpha \f$-parameter given by `alpha`.
 */
template <typename T, typename F, typename R = std::mt19937>
inline std::vector<vegas_iteration_result<T>> vegas(
