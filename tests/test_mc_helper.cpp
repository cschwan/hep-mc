...
TYPED_TEST(McHelper, CumulativeResult1WithOne)
{
    ...
    auto result = hep::accumulate<hep::weighted_equally>(one_result.begin(), one_result.end());
...
TYPED_TEST(McHelper, CumulativeResult1WithTwo)
{
    ...
    auto result = hep::accumulate<hep::weighted_equally>(two_results.begin(), two_results.end());
...
TYPED_TEST(McHelper, ChiSquareDof1WithZero)
{
    ...
    std::vector<hep::mc_result<T>> zero_results;
    T result = hep::chi_square_dof<hep::weighted_equally>(zero_results.begin(), zero_results.end());
...
TYPED_TEST(McHelper, ChiSquareDof1WithOne)
{
    ...
    T result = hep::chi_square_dof<hep::weighted_equally>(one_result.begin(), one_result.end());
...
TYPED_TEST(McHelper, DistributionAccumulator)
{
    ...
    std::vector<hep::plain_result<T>> results = {
        hep::plain_result<T>{it0, ir.non_zero_calls(), ir.finite_calls(), ir.calls(), ir.sum(),
            ir.sum_of_squares()},
        hep::plain_result<T>{it0, ir.non_zero_calls(), ir.finite_calls(), ir.calls(), ir.sum(),
            ir.sum_of_squares()}
    };
